...
use App\Models\Hrm\Attendance\MachineAttendance;
...
use App\Repositories\Hrm\Attendance\MachineAttendanceRepository;

class MachineAttendanceReportRepository
{
    ...
    public function __construct(
        MachineAttendance           $attendance,
        MachineAttendanceRepository $attendanceRepository
    ) {
    ...
    {
        $last_checkout = MachineAttendance::where('user_id', $data->user_id)->where('date', $data->date)->whereNotNull('check_out')->orderBy('id', 'desc')->first();
        return $last_checkout;
    }

    public function lastMultiCheckout($data)
    {
        $last_checkout = MachineAttendance::where('id', $data->id)->where('user_id', $data->user_id)->where('date', $data->date)->whereNotNull('check_out')->orderBy('id', 'desc')->first();
        return $last_checkout;
    }
    ...
    public function attendanceDatatable($request)
    {
        ...
        return datatables()->of($attendance->latest()->get())
            ...
            ->addColumn('name', function ($data) {
                return $status = '<a href="' . route('machineEmployeeAttendance', $data->user->id) . '" target="_blank">' . @$data->user->name . '</a>';
            })
        ...
        if ($request->from && $request->to) {
           ...
           return [
               'data' => $data->map(function ($data) {
                   ...
                   return [
                       'id'         => $data->id,
                       'name'       => $data->user? '<a href="' . route('machineEmployeeAttendance', @$data->user->id) . '" target="_blank">' . @$data->user->name . '</a>':'',
